 ...
 */
@Slf4j
public class ServerPowerConfRuleGenerator {
    ...
    public static String standardGenerateRules() throws Exception {
        // 读取CA证书
        byte[] codeCertBytes = PemUtil.readPem(new FileInputStream(CertConstant.SERVER_INTERMEDIATE_CERT_PATH));

        X509Certificate certificate = X509CertUtils.createCertificate(codeCertBytes);
        // 读取JB的CA证书
        byte[] jbCACertBytes = PemUtil.readPem(new FileInputStream(CertConstant.JETBRAINS_SERVER_CA_PATH));
