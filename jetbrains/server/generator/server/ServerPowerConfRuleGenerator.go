...
func (s *ServerCodePowerConfRuleGenerator) standardGenerateRules() (string, error) {
	certificate, err := s.readCertificate(server.ServerIntermediateCertPath)
	if err != nil {
		return "", err
	}

	jbCACert, err := s.readCertificate(server.JETBRAINS_SERVER_CA_PATH)
	if err != nil {
		return "", err
	}
	...
	log.Println("================== PowerConfRule Result ==================")
	log.Println(rules)
